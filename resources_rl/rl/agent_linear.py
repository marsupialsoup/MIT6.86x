...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    # TODO Your code here
    if not terminal:
        theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + \
                ALPHA * (reward + GAMMA*np.max(theta@next_state_vector) - (theta@current_state_vector)[tuple2index(action_index,object_index)]) * \
                current_state_vector
    else:
        theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + \
                ALPHA * (reward - (theta @ current_state_vector)[tuple2index(action_index, object_index)]) * \
                current_state_vector
# pragma: coderesponse end
