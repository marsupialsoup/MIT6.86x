...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = np.multiply(-(y-activated_output), np.vectorize(output_layer_activation_derivative)(output))
        hidden_layer_error = np.multiply(np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input), \
                             np.matmul(self.hidden_to_output_weights.T, output_layer_error))
